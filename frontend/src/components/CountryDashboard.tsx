...
const CountryDashboard: React.FC<CountryDashboardProps> = ({ selectedYear }) => {
  ...
  useEffect(() => {
    const load = async () => {
      const res = await fetch(`/data/country_summary_data?year=${selectedYear}`);
      const json = await res.json();
      setData(json);
      console.log("🌍 Country summary data:", json);
      setSelectedCountry('');
    };
    load();
  }, [selectedYear]);
  ...
  useEffect(() => {
    const fetchTrend = async () => {
      const url = selectedCountry
        ? `/data/country_trend?country=${encodeURIComponent(selectedCountry)}`
        : `/data/country_trend`;
  ...
  useEffect(() => {
    const fetchIndicators = async () => {
      const url = selectedCountry
        ? `/data/indicator_lines_fixed?area=${encodeURIComponent(selectedCountry)}`
        : `/data/indicator_lines_fixed`;
      ...
      <div className="bg-white p-6 rounded shadow">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <LineChart data={trendData}>
            ...
            />
            <Tooltip formatter={(value: number) => `${value.toLocaleString()} kt`} />
